...
namespace CH.Project.Commont.CacheCommont
{
    public static class MemoryCacheActionExtention
    {
        ...
        /// <param name="isSliding">是否滑动过期（如果在过期时间内有操作，则以当前时间点延长过期时间）</param>
        /// <returns></returns>
        public static bool Set(string key, object value, bool isSliding = false, bool ISaddRedis = false)
        {
            ...
            try
            {
                ...
                Cache.Set(key, value,
                    isSliding
                        ? new MemoryCacheEntryOptions().SetSlidingExpiration(ts)
                        : new MemoryCacheEntryOptions().SetAbsoluteExpiration(ts));
